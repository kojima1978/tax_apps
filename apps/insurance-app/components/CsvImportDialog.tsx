...
export default function CsvImportDialog({ caseId, onClose, onImported }: Props) {
  ...
  const handleDownloadTemplate = () => {
    const headers = [
      ...
      '被保険者', '受取人', '年金受取人', '死亡時の継続受取人', '年金受取開始方法', '年金受取開始年度', '年金受取年数', '死亡保障疾病', '死亡保障災害',
      '入院日額疾病', '入院日額災害', '診断一時金', '保険期間',
      '通貨', '為替レート', '契約時為替レート', '支払通貨', '一時払保険料実支払円額', '一時払保険料支払日',
      '保険料USD', '死亡保障疾病USD', '死亡保障災害USD',
      '入院日額疾病USD', '入院日額災害USD', '診断一時金USD', '満期保険金USD',
      '払方', '保険料', '払込終了年月日', '払込終了年齢', '払込終了済み', '満期保険金/年金原資',
      'コンサルタントメモ',
    ];
    const hints = [
      '※必須', '※必須：個人年金保険/収入保障保険/定期保険/がん保険/変額終身保険/医療保険/終身保険/養老保険', '任意', '※必須（個人年金・終身保険は任意）：YYYY-MM-DD', '自動計算可',
      '※必須：家族情報の氏名と一致', '任意：家族情報の氏名と一致',
      '個人年金のみ。未入力時は被保険者', '個人年金のみ・任意', '個人年金のみ：ageまたはfiscalYear', '年度指定時必須（例:2030）', '個人年金のみ必須（例:10）',
      '円（例:10000000）', '円',
      '円（例:10000）', '円', '円', '※必須：終身は999/有期は満了年齢',
      'JPYまたはUSD', '旧形式互換用。現在評価レートは基本情報で設定', '任意', 'ドル建て一時払のみ：JPYまたはUSD', '円払いのドル建て一時払のみ', 'ドル建て一時払のみ：YYYY-MM-DD',
      'USD建てのみ', 'USD建てのみ', 'USD建てのみ',
      'USD建てのみ', 'USD建てのみ', 'USD建てのみ', 'USD建てのみ',
      '※必須：monthly=月払/annual=年払/single=一時払', '※必須：円', 'YYYY-MM-DD', '歳 ※左と片方必須。個人年金は年金受取開始年齢', '任意：終了済みなら「済」', '円 ※個人年金は年金原資（受取総額）必須',
      '自由記述',
    ];
    const sample = [
      '住友生命', '終身保険', '1234567890', '2020-04-01', '30',
      '山田太郎', '山田花子', '', '', '', '', '', '10000000', '0',
      '0', '0', '0', '999',
      'JPY', '', '', '', '', '', '', '', '',
      '', '', '', '',
      'monthly', '15000', '', '60', '', '0',
