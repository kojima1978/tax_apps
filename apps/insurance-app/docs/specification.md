...
| 関数 | 説明 |
|---|---|
| `getCurrentDeathBenefit(policy, currentAge)` | 現在の死亡保障額。収入保障系は死亡保険金月額から残存期間の受取総額へ換算 |
...
| 現在の死亡保障額 | 有効証券の死亡保障合計。収入保障系は月額×残存期間で現在の受取総額に換算 |
