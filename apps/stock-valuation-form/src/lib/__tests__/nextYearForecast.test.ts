...
describe('calcNextYearForecast（繰り上がりによる否定的な断定）', () => {
  ...
  it('今期の利益だけで2年平均が基準を満たすなら、自動選択がその側を採るので必要額は0になる', () => {
    ...
    // 単年を明示指定すると2年平均の下駄が効かなくなり、来期の利益だけで200千円が要る
    const single = calcNextYearForecast(mkGetField(withTable4({ e18: '400', c1_mode: 'single' })));
    expect(single.elements[1]).toMatchObject({ key: 'C', required: 200 });
    expect(single.elements[1]!.requiredNote).toContain('「直前期の利益金額」を選択中');
  });
