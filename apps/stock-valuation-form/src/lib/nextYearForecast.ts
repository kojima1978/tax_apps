 ...
 *
 * 第4表の判定要素は毎期1年ずつ繰り上がる。来期の⑵（直前々期末を基準にしたⒷ₂・Ⓒ₂・Ⓓ₂）は、
 * 今期の⑴（Ⓑ₁・Ⓒ₁・Ⓓ₁）とまったく同じ値になる。したがって
 *  ・今期の⑴でゼロが2つ以上（医療法人は1つ以上）→ 来期の⑵側の条件は成立が確定
 *  ・今期の⑴でゼロがそれ未満              → 来期に比準要素数1の会社となることはあり得ない
 *
 * ここで見るのは判定要素のⒸ₁であって、類似業種比準価額の比準要素Ⓒではない
 * （単年／2年平均の選択は別々にできるので、両者の値は一致するとは限らない）。
...
// 見ているのは第4表の1の⑴（判定要素）なので添字つきで呼ぶ。
// Ⓒは比準要素として別に選べるようになり、Ⓒ₁と値が違いうるため区別が要る。
const B_LABEL = 'Ⓑ₁ 年配当金額';
const C_LABEL = 'Ⓒ₁ 年利益金額';
...
export function calcNextYearForecast(getField: TableProps['getField']): NextYearForecast {
  ...
  const reqC = cMode === 'single'
    ...
    : cMode === 'avg'
      ? reqCAvg
      : reqCSingle !== null && reqCAvg !== null ? Math.min(reqCSingle, reqCAvg) : null;
