...
function buildCells(j: Judgments): GridCell[] {
  ...
  { kind: 'label', text: '③の割合が50％以上である', highlightWhen: () => j.s2 === true, top: 22.87, left: 71.38, width: 11.32, height: 4.43 },
  { kind: 'label', text: '③の割合が50％未満である', highlightWhen: () => j.s2 === false, top: 22.78, left: 82.43, width: 11.59, height: 4.43 },
  ...
  { kind: 'label', text: '会 社 の 規 模 の 判 定（第１表の２の判定から自動）', top: 32.51, left: 67.7, width: 26.32, height: 3.86 },
  ...
  { kind: 'label', text: '･卸売業20億円以上･小売・サービス業15億円以上･上記以外の業種15億円以上', highlightWhen: () => j.landCol === 'smallA', top: 43.02, left: 63.74, width: 14.87, height: 8 },
  { kind: 'label', text: '･卸売業7,000万円以上20億円未満･小売・サービス業4,000万円以上15億円未満･上記以外の業種5,000万円以上15億円未満', highlightWhen: () => j.landCol === 'smallB', top: 43.02, left: 78.33, width: 15.68, height: 8 },
...
export function Table2Grid({ getField, updateField }: TableProps) {
  ...
  const g = (f: string): string => {
    switch (f) {
      case 'f16': return yenPart(t4.b1); case 'f17': return senPart(t4.b1);
      case 'f18': return fmt(t4.c1); case 'f19': return fmt(t4.d1);
      case 'f23': return yenPart(t4.b2); case 'f24': return senPart(t4.b2);
      case 'f25': return fmt(t4.c2); case 'f26': return fmt(t4.d2);
      case '①': return fmt(a01); case '②': return fmt(a02);
      case '③': return kabuRatio === null ? '' : String(kabuRatio);
      case '④': return fmt(a01); case '⑤': return fmt(a05);
      case '⑥': return landRatio === null ? '' : String(landRatio);
      case 'f96': return yenPart(t4.b1); case 'f97': return senPart(t4.b1);
      case 'f99': return fmt(t4.c1); case 'f101': return fmt(t4.d1);
    ...
    <span className="no-print" style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, whiteSpace: 'nowrap', flexWrap: 'wrap' }}>
      {([['s5_kaigyomae', '開業前'], ['s5_kyugyo', '休業中'], ['s6_seisan', '清算中']] as const).map(([key, label]) => (
        <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <input type="checkbox" checked={raw(key) === '1'} onChange={(e) => u(key, e.target.checked ? '1' : '')} />
          {label}
        </label>
      ))}
      <span style={{ fontWeight: 700 }}>判定結果：{RESULT_NAMES[result]}</span>
    </span>
  );
  return <GridForm cells={buildCells(j)} g={g} u={u} width="100%" title="第２表　特定の評価会社の判定の明細書" toolbar={toolbar} references={REFERENCES} />;
}
