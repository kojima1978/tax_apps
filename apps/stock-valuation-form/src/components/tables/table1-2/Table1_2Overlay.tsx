...
const IMAGE = `${BASE}/forms/table1-2.png`;
...
/** 第1表の2（背景画像オーバーレイ／CSSグリッド方式・測定用） */
export function Table1_2({ getField, updateField }: TableProps) {
  const g = (f: string) => getField(T, f);
  const u = (f: string, v: string) => updateField(T, f, v);
  return <FormOverlay image={IMAGE} width="100%" fields={FIELDS} g={g} u={u} picker={PICKER} storageKey="picker_table1_2" />;
}
