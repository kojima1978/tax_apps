...
/** ③④の注記。判定は Table5Grid の isExcludedLiability と対（記載方法等 第5表 2⑶）。 */
const EXCLUDED_LIABILITY_NOTE =
  '\n科目名に「引当金」「準備金」「繰延税金負債」を含む行は、純資産価額計算上の負債に含めません（記載方法等 第５表 2⑶）';
...
export function table5Hints(d: Detail): Record<string, string> {
  ...
  return {
    ...
    '③': `負債の部の「相続税評価額」欄の合計 ${rv(d.liabilityEval)}千円${EXCLUDED_LIABILITY_NOTE}`,
    '④': `負債の部の「帳簿価額」欄の合計 ${rv(d.liabilityBook)}千円${EXCLUDED_LIABILITY_NOTE}`,

    '⑤': `① ${rv(d.assetEval)} － ③ ${rv(d.liabilityEval)} ＝ ${rv(d.netEval)}千円`,
    ...
    '⑧': d.specialMarketValueRules
      ...
      : `⑦ ${rv(d.evaluationDifference)}千円 × ${formatRatePercent(d.corporateTaxRatePercent)}％ ＝ ${rv(d.corporateTaxEquivalent)}千円（端数切捨て）`
        + (d.corporateTaxOverridden
          ? '\n率は「（⑦×○％）」に入力された値です（空欄に戻すと課税時期から決まる率に戻ります）'
          : '\n率は課税時期から決まります。「（⑦×○％）」の○を書き換えると任意の率で計算できます'),
    ...
    '⑪': d.currentNet < 0
      ? `⑨ ${rv(d.currentNet)}千円がマイナス（債務超過）のため、0円とします（株式にマイナスの評価額は付かないため）`
      : `⑨ ${rv(d.currentNet)}千円 × 1,000 ÷ ⑩ ${rv(d.currentShares)}株 ＝ ${rd(d.netPerShareDisp)}円`
        + `\n${FRACTION_NOTE_TAX_TIME}`,
    '⑫': d.votingRatio === null
      ? '第１表の１の議決権の数がまだ入っていないため、50％以下かどうかを判定できません'
      : d.votingRatio > 50
        ? `同族株主等の議決権割合が ${pct(d.votingRatio)}％（50％超）のため、この欄は記載しません`
        : `⑪ ${rd(d.netPerShareDisp)}円 × 80％ ＝ ${rd(d.netPerShare80Disp)}円`
          + `\n同族株主等の議決権割合 ${pct(d.votingRatio)}％（50％以下）のため記載します`
          + `\n${FRACTION_NOTE_TAX_TIME}`,
  };
}
