...
describe('通し検算（日本サンプル工業株式会社・中会社・一般の評価会社）', () => {
  ...
  describe('第5表（1株当たりの純資産価額）', () => {
    ...
    // 課税時期が令和8年3月15日（4月1日より前）なので率は37％。
    // ここが38％に変わると以降の第3表まで全部ずれるので、境目はこの通し検算でも押さえる
    it('⑧法人税額等相当額＝125,000×37%＝46,250千円', () => {
      expect(t5['⑧']).toBe(46250);
    });

    it('⑨課税時期現在の純資産価額＝255,000－46,250＝208,750千円', () => {
      expect(t5['⑨']).toBe(208750);
    });

    it('⑩⑪＝208,750千円×1,000÷20,000株＝10,437円', () => {
      expect(t5['⑩']).toBe(20000);
      expect(t5['⑪']).toBe(10437);
    });
  ...
  describe('第3表（1株当たりの株式の価額）', () => {
    ...
    it('⑤中会社＝4,715×0.75＋10,437×0.25＝6,145円', () => {
      expect(t3.v5).toBe(6145);
    });

    it('④大会社・⑥小会社も併せて確認（会社規模が動いたときの比較用）', () => {
      expect(t3.v4).toBe(4715);  // min(比準4,715, 純資産10,437)
      expect(t3.v6).toBe(7576);  // min(10,437, 4,715×0.5＋10,437×0.5)
    });

    it('修正がないので原則的評価方式の価額は⑤のまま', () => {
      expect(t3.v8).toBeNull();
      expect(t3.v12).toBeNull();
      expect(t3.gensoku).toBe(6145);
    });
    ...
    it('配当還元方式（適用はしないが様式上は計算される）', () => {
      ...
      expect(t3.v24).toBe(1250);   // 原則的評価額6,145を超えないのでそのまま
    });

    it('同族株主等なので原則的評価方式＝6,145円が最終価額', () => {
      expect(t3.useHaito).toBe(false);
      expect(t3.finalPrice).toBe(6145);
    });
  });
});
