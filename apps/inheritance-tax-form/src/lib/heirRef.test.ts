  ...
  for (let page = 0; page < 2; page += 1) {
    cells.push(
      ...
      ...buildTable13(COMMON, TOTALS, {
        ...
        debt: Array.from({ length: TABLE13_DEBT_ROWS }, (_, r) => ({
          prefix: `${TABLE13_DEBT_FORM}#${page * TABLE13_DEBT_ROWS + r}.`, label: `債務${r}`,
        })),
        funeral: Array.from({ length: TABLE13_FUNERAL_ROWS }, (_, r) => ({
          prefix: `${TABLE13_FUNERAL_FORM}#${page * TABLE13_FUNERAL_ROWS + r}.`, label: `葬式費用${r}`,
        })),
      }, false, OPTIONS),
    );
  }
...
describe('参照の解決', () => {
  it('IDを何人目かに直して計算へ渡す', () => {
    const common: Values = { t9d0Who: id(2), t10d0Who: id(0), name: '被相続人' };
    const details = { table11f1: [{ who0: id(1), value: '100' }] };
    const out = resolveHeirRefs(common, heirs, details);
    expect(out.common).toEqual({ t9d0Who: '3', t10d0Who: '1', name: '被相続人' });
    expect(out.details.table11f1).toEqual([{ who0: '2', value: '100' }]);
  });

  it('並べ替えても同じ人を指す（番号だけが変わる）', () => {
    const common: Values = { t10d0Who: id(2) };
    const swapped = [heirs[2]!, heirs[0]!, heirs[1]!];
    expect(resolveHeirRefs(common, heirs, {}).common.t10d0Who).toBe('3');
    expect(resolveHeirRefs(common, swapped, {}).common.t10d0Who).toBe('1');
  });

  it('消された人を指していた欄は空欄になる', () => {
    const common: Values = { t10d0Who: id(1) };
    expect(resolveHeirRefs(common, [heirs[0]!, heirs[2]!], {}).common.t10d0Who).toBe('');
  });
});

describe('版5からの移行', () => {
  it('「何人目か」をその人のIDに読み替える', () => {
    const common: Values = { t10d0Who: '3', t4no: '1' };
    const details = { table11f1: [{ who0: '2' }] };
    const out = migrateHeirRefs(common, heirs, details);
    expect(out.common).toEqual({ t10d0Who: id(2), t4no: id(0) });
    expect(out.details.table11f1).toEqual([{ who0: id(1) }]);
  });

  it('居ない人を指していた欄は空欄になる', () => {
    expect(migrateHeirRefs({ t10d0Who: '9' }, heirs, {}).common.t10d0Who).toBe('');
  });
});
