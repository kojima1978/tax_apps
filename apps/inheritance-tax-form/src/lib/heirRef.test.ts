...
import { TABLE9_DETAIL_FORM, TABLE9_ROWS, buildTable9 } from '../forms/table9';
import { TABLE10_DETAIL_FORM, TABLE10_ROWS, buildTable10 } from '../forms/table10';
import {
...
/** 明細の行（配列の要素）を指す接頭辞 */
function rows(form: string, page: number, count: number) {
  return Array.from({ length: count }, (_unused, r) => {
    const index = page * count + r;
    return { prefix: `${form}#${index}.`, label: `明細${index + 1}` };
  });
}
  ...
  for (let page = 0; page < 2; page += 1) {
    cells.push(
      ...
      ...buildTable13(COMMON, TOTALS, {
        ...
        debt: rows(TABLE13_DEBT_FORM, page, TABLE13_DEBT_ROWS),
        funeral: rows(TABLE13_FUNERAL_FORM, page, TABLE13_FUNERAL_ROWS),
      }, false, OPTIONS),
    );
  }
  cells.push(
...
describe('参照の解決', () => {
  it('IDを何人目かに直して計算へ渡す', () => {
    const common: Values = { t14p0Who: id(2), t14p1Who: id(0), name: '被相続人' };
    const details = { table11f1: [{ who0: id(1), value: '100' }] };
    const out = resolveHeirRefs(common, heirs, details);
    expect(out.common).toEqual({ t14p0Who: '3', t14p1Who: '1', name: '被相続人' });
    expect(out.details.table11f1).toEqual([{ who0: '2', value: '100' }]);
  });

  it('並べ替えても同じ人を指す（番号だけが変わる）', () => {
    const common: Values = { t14p1Who: id(2) };
    const swapped = [heirs[2]!, heirs[0]!, heirs[1]!];
    expect(resolveHeirRefs(common, heirs, {}).common.t14p1Who).toBe('3');
    expect(resolveHeirRefs(common, swapped, {}).common.t14p1Who).toBe('1');
  });

  it('消された人を指していた欄は空欄になる', () => {
    const common: Values = { t14p1Who: id(1) };
    expect(resolveHeirRefs(common, [heirs[0]!, heirs[2]!], {}).common.t14p1Who).toBe('');
  });
});

describe('版5からの移行', () => {
  it('「何人目か」をその人のIDに読み替える', () => {
    const common: Values = { t14p1Who: '3', t4no: '1' };
    const details = { table11f1: [{ who0: '2' }] };
    const out = migrateHeirRefs(common, heirs, details);
    expect(out.common).toEqual({ t14p1Who: id(2), t4no: id(0) });
    expect(out.details.table11f1).toEqual([{ who0: id(1) }]);
  });

  it('居ない人を指していた欄は空欄になる', () => {
    expect(migrateHeirRefs({ t14p1Who: '9' }, heirs, {}).common.t14p1Who).toBe('');
  });
});
