...
const emptyData = (): FormData => ({ common: {}, heirs: [{}] });
...
function loadStored(): FormData {
  try {
    ...
    return { common: parsed.common, heirs: parsed.heirs.length > 0 ? parsed.heirs : [{}] };
  } catch {
    return emptyData();
  }
}
...
export function useFormData() {
  ...
  const computed = useMemo(() => computeAll(data.common, data.heirs), [data]);
  ...
  const exportJson = useCallback(() => {
    ...
    link.href = url;
    link.download = `相続税申告書_第1表_${new Date().toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  const importJson = useCallback(async (file: File): Promise<boolean> => {
    try {
      ...
      setData({ common: parsed.common, heirs: parsed.heirs.length > 0 ? parsed.heirs : [{}] });
      return true;
    } catch {
      return false;
    }
  ...
  return { data, g, u, addHeir, removeHeir, reset, exportJson, importJson, maxHeirs: MAX_HEIRS };
}
