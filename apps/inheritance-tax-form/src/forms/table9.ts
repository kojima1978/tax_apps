...
const NOTES1: NonNullable<GridCell['numberedNotes']> = [
  {
    number: '1',
    body: '相続人（相続の放棄をした人を除きます。以下同じです。）が受け取った保険金などのうち一定の金額は'
      + '非課税となりますので、その人は、次の2の該当欄に非課税となる金額と課税される金額とを記入します。',
  },
  {
    number: '2',
    body: '相続人以外の人が受け取った保険金などについては、非課税となる金額はありませんので、その人は、'
      + 'その受け取った金額そのままを第11表の付表4の「財産の明細」の「価額」欄に転記します。',
  },
  { number: '3', body: '相続時精算課税適用財産は含まれません。' },
];
  ...
  return [
    ...
    ...Array.from({ length: TABLE9_ROWS }, (_unused, r): GridCell[] => {
      ...
      return [
        ...
        mk(y, col(X.C1, X.ADDR), {
          kind: 'input', field: `${totals}t9r${i}No`, ariaLabel: `${who}の氏名`,
          options: whoOptions, align: 'left', readOnly: true,
        }),
        code(y, col(X.ADDR, X.C2), cd('G', g)),
        mk(y, col(X.C2, P.V1_R), {
          kind: 'input', field: `${totals}t9r${i}v1`, ariaLabel: `${who}が受け取った保険金などの金額`,
...
export function buildTable9(
  ...
  return [
    ...
    mk(row(1636.5, BOTTOM), col(X.L, X.R), {
