...
const TABLE11_LIST_NOTES = '（注）　1　「遺産の分割状況」欄は、遺産の分割状況に応じた番号を記入します。\n'
  + '　　　　2　「分割の日」欄は、遺産の全部又は一部について分割がされている場合には、その分割の日を記入します。';

/** 2「取得財産の価額の合計表」の（注） */
const TABLE11_SUM_NOTES = '（注）　1　「財産を取得した人の番号」欄は、上記1の「項番」欄に記入した番号を記入します。\n'
  + '　　　　2　「①分割財産の価額」欄は、第11表の付表1から付表4の「分割が確定した財産」の「取得財産の価額」欄に'
  + '記入した価額について、財産を取得した人ごとに合計した金額を記入します。\n'
  + '　　　　3　「②未分割財産の価額」欄は、第11表の付表1から付表4の「財産の明細」に記入した財産のうち、'
  + '未分割である財産の価額の合計額を各相続人が相続分（寄与分を除きます。）に応じて取得するとした場合に計算される金額を記入します。\n'
  + '　　　　4　「③取得財産の価額」欄の金額を第1表のその人の「取得財産の価額①」欄に転記します。';
  ...
  return [
    ...
    ...people.flatMap((person, slot): GridCell[] => {
      ...
      return [
        ...
        mk(r, col(X.E_L, X.SUM2_L), { kind: 'input', field: `${p}t11v1`, ariaLabel: `${person.label} ①分割財産の価額`, commaInteger: true }),
