 ...
 * 1の明細は行を並べ替えられるよう「行そのもの」の配列（`details`）に持ち、
 * 2は1からの導出で手入力が無いので自動計算欄（'t.' スコープ）。
 * どちらも人ごとの欄ではないので、枚数は共通欄 `t10Pages` で表全体を1つとして決める。
...
/**
 * 1 退職手当金などの明細を持つ配列のID（`FormData.details` のキー）。
 * 行そのものを1要素として持つ理由は第9表と同じ（並べ替えが要素の移動だけで済む）。
 */
export const TABLE10_DETAIL_FORM = 'table10detail';
...
export const TABLE10_ASPECT = aspect;
...
/** ②の見出しに組まれている分数「各人の①／Ⓑ」の位置（横線は実測 y=1157・x=834〜890） */
const FRAC = { L: 834, R: 890, BAR: 1157 } as const;
...
function personRows(totals: string, page: number, last: boolean, whoOptions: GridCell['options']): GridCell[] {
  ...
  return [
    ...
    label(row(1128, FRAC.BAR - FRACTION_BAR_DY), col(FRAC.L, FRAC.R), '各人の①', { noBorderLeft: true, noBorderRight: true, noBorderBottom: true }),
