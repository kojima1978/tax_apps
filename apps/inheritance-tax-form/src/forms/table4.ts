...
const NOTES: NonNullable<GridCell['numberedNotes']> = [
  {
    number: '1',
    body: '相続時精算課税適用者である孫が相続開始の時までに被相続人の養子となった場合は、'
      + '「相続時精算課税に係る贈与を受けている人で、かつ、相続開始の時までに被相続人との続柄に変更があった場合」'
      + 'には含まれませんので②欄から④欄までの記入は不要です。',
  },
  {
    number: '2',
    body: '②欄には、次に掲げる場合の区分に応じ、それぞれ次の金額の合計額を記入します。\n'
  ...
  {
    number: '3',
    body: '各人の⑥欄の金額を第1表のその人の「相続税額の2割加算が行われる場合の加算金額⑪」欄に転記します。',
  },
];
  ...
  return [
    ...
    ...ROWS.flatMap((def): GridCell[] => {
      ...
      return [
        ...
        label(y, col(X.LBL, X.NUM), def.no, {
          fontSize: 10, semanticRole: 'rowheader', flexDirection: 'column', align: 'center',
        }),
        ...P.flatMap(([codeL, codeR, valueR], j): GridCell[] => [
    ...
    mk(row(Y.notes[0], Y.notes[1]), col(LEFT, RIGHT), {
      kind: 'label', numberedNotes: NOTES, align: 'left', alignItems: 'flex-start', fontSize: 6.5,
    }),
  ];
}
