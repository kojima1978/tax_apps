import { giftData, taxReturnData, replaceYearPlaceholder, type TaxType, type DocumentGroup, type EditableCategory, type EditableDocument, type EditableDocumentList } from '@/constants';
...
// 贈与税データから編集可能なリストを初期化
const initializeGiftTax = (): EditableDocumentList => [
...
// 確定申告データから編集可能なリストを初期化
const initializeIncomeTax = (year: number): EditableDocumentList => {
  const replaceYear = (docs: string[]) =>
    toEditableDocuments(docs.map(d => replaceYearPlaceholder(d, year)));

  return [
    ...taxReturnData.baseRequired.map((base, i): EditableCategory => ({
      id: `base_${i}`,
      name: base.category,
      documents: replaceYear(base.documents),
      note: base.note,
      isExpanded: true,
      isSpecial: false,
    })),
    ...taxReturnData.options.map((opt): EditableCategory => ({
      id: `option_${opt.id}`,
      name: `【所得】${opt.label}`,
      documents: replaceYear(opt.documents),
      isExpanded: true,
      isSpecial: false,
    })),
    ...taxReturnData.deductions.map((ded): EditableCategory => ({
      id: `deduction_${ded.id}`,
      name: `【控除】${ded.label}`,
      documents: replaceYear(ded.documents),
      isExpanded: true,
      isSpecial: true,
    })),
  ];
...
// 申告種別に応じた編集可能なリストを初期化
export const initializeEditableList = (taxType: TaxType, year: number): EditableDocumentList =>
  taxType === 'income-tax' ? initializeIncomeTax(year) : initializeGiftTax();
