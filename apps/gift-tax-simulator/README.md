...
- 相続の場合は非課税メッセージ表示
- 印刷2P目: Q14（住宅用土地の税額軽減）・Q13（建物控除テーブル）の参照資料
...
- 土地・建物の持ち分入力（分数形式 例: 1/2）→ 課税標準（評価額×持ち分）に反映
- 計算過程の詳細表示（トグル）、印刷時は常に展開
- 印刷2P目: No.7191 税額表（土地・建物・住宅用家屋軽減税率・端数処理）の参照資料

### 共通機能
- 取得税⇔登録免許税間の評価額・持ち分を一括引用（localStorage経由）
- 印刷: 会社情報をページ先頭に常時表示（結果入力前でも表示）
- 印刷: 持ち分入力欄をフラクション形式で表示
- 印刷: 結果ボックスを白黒対応（白背景＋緑ボーダー＋黒文字）
- 印刷: zoom: 0.82 でA4縮小（ページブレーク計算が正確）・入力値の視認性向上（下線＋太字）
- 印刷用フッター（担当者名入力・作成日）
- ポータルに戻るリンク（ヘッダー左端）
