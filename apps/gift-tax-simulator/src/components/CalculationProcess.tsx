...
type StepDef = {
    label: string;
    getValue: (d: CalculationResult['detail'], isTaxFree: boolean) => string;
    isResult?: boolean;
};
...
const STEPS: StepDef[] = [
    {
        label: '① 1回あたりの贈与金額',
        getValue: (d) => `${formatCurrency(d.giftAmount)} 円`,
    },
    {
        label: '② 基礎控除額',
        getValue: () => `△ ${formatCurrency(BASIC_DEDUCTION)} 円`,
    },
    {
        label: '③ 課税価格 （① − ②）',
        getValue: (d, isTaxFree) =>
                    ...
                    >
                        ...
                        <div className="calc-step-body">
                            {STEPS.map((step) => (
                                <div
                                    key={step.label}
                                    className={`calc-step-row ${step.isResult ? 'calc-step-row-result' : ''}`}
                                >
                                    <span className="calc-step-label">{step.label}</span>
                                    <span className="calc-step-value">
                                        {step.getValue(d, isTaxFree)}
                                    </span>
                                </div>
                            ))}
