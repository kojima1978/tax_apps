...
| STEP | 内容 |
|:-----|:-----|
| STEP0 | 基本情報（医療法人名・担当者・事業年度の選択）※登録済み最新年度を自動選択 |
| STEP1 | 医療法人規模の判定（正職員数・総資産・売上高） |
| STEP2 | 財務データ入力（決算書より純資産・利益を入力）※入力不要セルは「―」表示 |
...
- **参考要素一覧**: 医療法人規模・類似業種比準価額・純資産価額・L値・評価方式
...
│   ├── company-settings/page.tsx           # 医療法人マスタ設定
...
│   └── api/                                # APIルート
│       ├── companies/route.ts              # 医療法人CRUD API
...
│   ├── ResultsExcelExport.tsx              # Excel出力（データ駆動セクションビルダー）
...
│   └── valuation/                          # 評価入力ステップ
│       ├── Step0BasicInfo.tsx              # 基本情報入力（年度自動選択）
│       ├── Step1CompanySize.tsx            # 医療法人規模判定
│       ├── Step2FinancialData.tsx          # 財務データ入力（DisabledCell）
│       └── Step3Investors.tsx              # 出資者情報入力
├── hooks/
│   ├── useFormData.ts                      # フォームデータ管理（14 state + handlers）
│   ├── useSavedData.ts                     # 保存データ管理（CRUD + JSON export/import）
│   ├── useMasterSettings.ts               # マスタ設定共通hook（fetchList利用）
...
│   ├── constants.ts                        # 定数定義
│   ├── utils.ts                            # 汎用ユーティリティ（formatSen/formatYen/fetchList）
