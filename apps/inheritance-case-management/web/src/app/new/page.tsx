"use client"

import { EditCaseForm } from "../[id]/edit-case-form"
import { Suspense } from "react"
import type { InheritanceCase } from "@/types/shared"
import Link from "next/link"
import { ChevronRight } from "lucide-react"

const emptyCase: InheritanceCase = {
    id: 0, // Will be generated by DB
    deceasedName: "",
    dateOfDeath: new Date().toISOString().split("T")[0],
    status: "未着手",
    taxAmount: 0,
    assigneeId: null,
    feeAmount: 0,
    fiscalYear: new Date().getFullYear(),
    referrerId: null,
    estimateAmount: 0,
    propertyValue: 0,
    progress: [
        { id: 0, stepId: "step-1", name: "初回連絡", sortOrder: 0, date: null },
        { id: 0, stepId: "step-2", name: "初回面談", sortOrder: 1, date: null },
        { id: 0, stepId: "step-3", name: "2回目訪問", sortOrder: 2, date: null },
        { id: 0, stepId: "step-8", name: "最終チェック完了", sortOrder: 3, date: null },
        { id: 0, stepId: "step-4", name: "遺産分割（済）", sortOrder: 4, date: null },
        { id: 0, stepId: "step-5", name: "申告（済）", sortOrder: 5, date: null },
        { id: 0, stepId: "step-6", name: "請求（済）", sortOrder: 6, date: null },
        { id: 0, stepId: "step-7", name: "入金確認", sortOrder: 7, date: null },
    ],
    contacts: [{ id: 0, sortOrder: 0, name: "", phone: "", email: "" }],
}

export default function NewCasePage() {
    return (
        <div className="container mx-auto py-10 max-w-5xl px-4">
            <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-6">
                <Link href="/" className="hover:text-foreground transition-colors">案件一覧</Link>
                <ChevronRight className="h-3.5 w-3.5" />
                <span className="text-foreground font-medium">新規案件登録</span>
            </nav>

            <div className="bg-card text-card-foreground rounded-lg border shadow-sm p-6">
                <div className="mb-8 border-b pb-4">
                    <h1 className="text-2xl font-bold tracking-tight">新規案件登録</h1>
                    <p className="text-muted-foreground">基本情報を入力して案件を登録します</p>
                </div>

                <Suspense fallback={<div>Loading...</div>}>
                    <EditCaseForm initialData={emptyCase} isCreateMode={true} />
                </Suspense>
            </div>
        </div>
    )
}
