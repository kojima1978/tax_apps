...
- **経営分析**: 年度別集計、担当者別ランキング、紹介者別実績
- **CSV取込・出力**: 案件データのCSVインポート/エクスポート
- **リアルタイムバリデーション**: Zod による入力検証

### UI/UX

- **ステータスバッジ**: 案件ステータス（未着手/進行中/完了/請求済）・受託状況を色付きバッジで視覚的に表示
- **進捗バー**: 案件一覧テーブルにミニプログレスバーを表示し、進捗率を一目で把握
- **フィルターチップ**: 適用中のフィルター条件をチップ表示し、個別に解除可能
- **タイムライン進捗エディタ**: 案件詳細の進捗工程を縦タイムラインUIで表示（完了=緑ドット、未完了=グレードット）
- **進捗モーダル**: 一覧画面からワンクリックで進捗日付を編集・保存
- **KPIダッシュボード**: 案件総数・進行中・期限間近・当月完了の4指標をカード表示
