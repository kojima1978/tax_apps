...
- 相続税案件の作成・閲覧・編集・削除
- ステータス管理: 未着手 / 進行中 / 完了（税務申告済） / 入金済 / 対応終了
- 受託状況管理: 受託可 / 受託不可 / 未判定 / 保留
- 受託状況とステータスの連動ルール:
  - 未判定・保留 → ステータスは「未着手」のみ選択可
  - 受託不可 → ステータスは自動的に「対応終了」に設定
  - 受託可 → 未着手 / 進行中 / 完了（税務申告済） / 入金済 を選択可
- 担当者・紹介者のリレーション（FK）
...
- 一覧画面の進捗列に最終完了工程名を表示
- 進捗ステップの日付入力に応じたステータス自動変更提案:
  - 「申告（済）」に日付入力 → 「完了（税務申告済）」への変更を確認
  - 「入金確認」に日付入力 → 「入金済」への変更を確認
...
- **紹介者**: 会社別実績ランキング + 紹介者別実績ランキング（報酬総額/件数、ソート可能）
- 年度セレクターで全年度/特定年度を切替
...
- **ステータスバッジ**: 案件ステータス（灰=未着手/青=進行中/緑=完了（税務申告済）/紫=入金済/赤=対応終了）、受託状況（緑/赤/灰/琥珀）を色付きバッジで表示
...
- **フィルター**: 年度・受託状況・進行・部門・担当者・検索をdata-driven定義（`FILTER_KEYS`/`STATIC_FILTER_DEFS`）
...
| status | string | ステータスフィルタ（未着手/進行中/完了（税務申告済）/入金済） |
...
erDiagram
    ...
    InheritanceCase {
        ...
        string status "ステータス（未着手/進行中/完了（税務申告済）/入金済/対応終了）"
...
- **進捗→ステータス連動**: `STATUS_PROMPT_RULES` でdata-driven化、進捗保存時にルールに基づきステータス変更を提案
