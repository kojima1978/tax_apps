...
- 相続税案件の作成・閲覧・編集・削除
- ステータス管理: 未着手 / 手続中 / 申告済 / 請求済 / 入金済 / 対応終了
...
- 一覧画面の進捗列に最終完了工程名を表示
- 進捗ステップの日付入力に応じたステータス自動変更提案（`STATUS_PROMPT_RULES` + `STEP_NAMES` 定数）:
  - 「申告（済）」に日付入力 → 「申告済」への変更を確認
  - 「請求（済）」に日付入力 → 「請求済」への変更を確認
  - 「入金（済）」に日付入力 → 「入金済」への変更を確認
...
- 年度セレクター（デフォルト: 現在年度、新しい順）で全年度/特定年度を切替
...
- **ステータスバッジ**: 案件ステータス（灰=未着手/青=手続中/緑=申告済/橙=請求済/紫=入金済/赤=対応終了）、受託状況（緑/赤/灰/琥珀）を色付きバッジで表示
...
- **進捗→ステータス連動**: `STATUS_PROMPT_RULES` + `STEP_NAMES` 定数でdata-driven化、進捗保存時にルールに基づきステータス変更を提案
