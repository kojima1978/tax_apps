...
export const TaxBracketTable: React.FC<TaxBracketTableProps> = memo(({ compact }) => {
  const heading = compact ? 'h3' : 'h2';
  const Heading = heading;

  const content = (
    <>
      <Heading className={`${compact ? 'text-base' : 'text-xl'} font-bold text-gray-800 mb-1`} id="bracket-heading">相続税の速算表</Heading>
  ...
  if (compact) {
    return <div className="overflow-x-auto" role="region" aria-label="相続税の速算表">{content}</div>;
  }
