...
import { formatCurrency, formatPercent, countEffectiveHeirs } from '../utils';
...
export const ExcelExport: React.FC<ExcelExportProps> = memo(({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // ヘッダーセルの共通スタイル適用
      const greenBorder = { style: 'medium' as const, color: { argb: 'FF16A34A' } };
      const applyMainHeaderStyle = (cell: ReturnType<typeof worksheet.getCell>) => {
        cell.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 12 };
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF16A34A' } };
        cell.alignment = { vertical: 'middle', horizontal: 'center' };
        cell.border = { top: greenBorder, left: greenBorder, bottom: greenBorder, right: greenBorder };
      };
