...
const heirRankLabels: Record<Portfolio["planning"]["heirRank"], string> = {
  none: "なし",
  rank1: "子",
  rank2: "親・祖父母",
  rank3: "兄弟姉妹",
...
export function ForecastModal({ planning, members, onClose, onSubmit, saving }: { planning: Portfolio["planning"]; members: Portfolio["familyMembers"]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; saving: boolean }) {
  const hasFamilyDetail = members.length > 0;
  ...
  return <div className="modal-layer" role="presentation"><div className="modal forecast-modal" role="dialog" aria-modal="true" aria-labelledby="forecast-modal-title"><header><div><p className="eyebrow">INHERITANCE TAX</p><h2 id="forecast-modal-title">相続税計算の家族情報</h2></div><button className="icon-button" aria-label="閉じる" onClick={onClose}><X /></button></header><form onSubmit={onSubmit}>{hasFamilyDetail
    ? <><p className="form-intro">相続人は<strong>親族関係タブ</strong>の登録内容から自動で反映します。人数や続柄を変更する場合は親族関係タブで編集してください。</p><dl className="forecast-family-summary"><div><dt>配偶者</dt><dd>{planning.hasSpouse ? "あり" : "なし"}</dd></div><div><dt>配偶者以外の相続人</dt><dd>{planning.heirRank === "none" ? "なし" : `${heirRankLabels[planning.heirRank]} ${planning.heirCount}人`}</dd></div></dl></>
    : <><p className="form-intro">概算に必要な最小限の情報です。法定相続分で計算し、代襲相続などの詳細は計算画面で調整できます。親族関係タブに相続人を登録すると、この欄は自動で反映されます。</p><div className="family-form-grid"><label>配偶者<select name="hasSpouse" value={String(hasSpouse)} onChange={(event) => changeSpouse(event.target.value === "true")}><option value="false">なし</option><option value="true">あり</option></select></label><label>配偶者以外の相続人<select name="heirRank" value={heirRank} onChange={(event) => setHeirRank(event.target.value as Portfolio["planning"]["heirRank"])}><option value="rank1">子</option><option value="rank2">親・祖父母</option><option value="rank3">兄弟姉妹</option><option value="none" disabled={!hasSpouse}>なし</option></select></label>{heirRank !== "none" ? <label>人数<input name="heirCount" type="number" min="1" max="20" step="1" defaultValue={Math.max(1, planning.heirCount)} required /></label> : <input type="hidden" name="heirCount" value="0" />}</div></>
