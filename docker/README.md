...
| DBバックアップ | `backup-db.bat` | ITCM PostgreSQLダンプ + Excelテンプレートバックアップ（7日間保持） |
...
`backup-db.bat` を使って ITCM PostgreSQL + Excelテンプレートの定期バックアップを設定できます。
...
| 方式② | `robocopy`（`*.xlsx` フィルタ） |
| 保存先② | `docker\backups\itcm-templates\itcm-templates_YYYYMMDD_HHMM\` |
