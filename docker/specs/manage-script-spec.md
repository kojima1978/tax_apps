...
**最終更新**: 2026-04-16
**バージョン**: 2.4（backup/restore データ駆動化リファクタリング）
...
| # | 変数名 | パス | 説明 | ポート |
|:--|:-------|:-----|:-----|:-------|
| 1 | `APP_1` | `apps\inheritance-case-management` | 案件管理 (PostgreSQL + Next.js + Hono API) | 3020/3022 |
| 2 | `APP_2` | `apps\bank-analyzer-django` | 銀行分析 (PostgreSQL + Django) | 3007 |
| 3 | `APP_3` | `apps\tax-docs` | 所得税・贈与税書類リスト (SQLite + Express + Vite) | 3002 |
| 4 | `APP_4` | `apps\medical-stock-valuation` | 医療法人株式 (SQLite + Next.js) | 3010 |
| 5 | `APP_5` | `apps\shares-valuation` | 非上場株式 (Next.js) | 3012 |
...
backup/restore コマンドで扱うデータソースは、定数配列（manage.sh）/ 番号付き変数（manage.bat）でデータ駆動的に定義されている。新しいバックアップ対象を追加する際は、この定義に1行追加するだけで backup/restore 両方に反映される。

### 4.1 PostgreSQL ターゲット

| manage.sh | manage.bat |
|:----------|:-----------|
| `PG_TARGETS` 配列（コロン区切り） | `PG_COUNT` + `PG_1`〜`PG_N`（パイプ区切り） |
...
| manage.sh | manage.bat |
|:----------|:-----------|
| `SQLITE_TARGETS` 配列 | `SQLITE_COUNT` + `SQLITE_1`〜`SQLITE_N` |
...
| manage.sh | manage.bat |
|:----------|:-----------|
| `BIND_TARGETS` 配列 | `BIND_COUNT` + `BIND_1`〜`BIND_N` |
...
| manage.sh | manage.bat |
|:----------|:-----------|
| `SETTINGS_TARGETS` 配列 | `SETTINGS_COUNT` + `SETTINGS_1`〜`SETTINGS_N` |
...
| ラベル | 引数 | 戻り値 | 説明 |
|:-------|:-----|:-------|:-----|
| `:do_start_app` | `%1`=app_index | - | 単一アプリ起動（開発/本番分岐、.env自動生成） |
| `:do_compose_action` | `%1`=app_index, `%2`=action | - | 単一アプリに対する docker compose コマンド実行 |
...
| `:do_backup_postgres` | `%1`〜`%7` | `BACKUP_OK`/`BACKUP_FAIL`/`BACKUP_SKIP` | PostgreSQL バックアップ（pg_dump → volume fallback） |
| `:do_restore_sqlite` | `%1`=filename, `%2`=volume | `SQLITE_OK`/`RESTORE_FAIL` | SQLite ボリュームを tar.gz からリストア |
| `:do_restore_postgres` | `%1`〜`%8` | `RESTORE_OK`/`RESTORE_FAIL`/`RESTORE_SKIP` | PostgreSQL リストア（SQL → volume fallback） |
...
| `:do_backup_postgres_target` | `%1`=PG index | - | `PG_%1` をパースして `:do_backup_postgres` にディスパッチ |
| `:do_backup_sqlite_target` | `%1`=SQLITE index | - | `SQLITE_%1` をパースして `:do_backup_sqlite` にディスパッチ |
| `:do_backup_bind_target` | `%1`=BIND index | `BACKUP_OK`/`BACKUP_FAIL`/`BACKUP_SKIP` | `BIND_%1` をパースしてバインドマウントデータをバックアップ |
| `:do_backup_settings_target` | `%1`=SETTINGS index | `BACKUP_OK`/`BACKUP_SKIP` | `SETTINGS_%1` をパースして設定ファイルをバックアップ |
| `:do_restore_postgres_target` | `%1`=PG index | - | `PG_%1` をパースして `:do_restore_postgres` にディスパッチ |
| `:do_restore_sqlite_target` | `%1`=SQLITE index | - | `SQLITE_%1` をパースして `:do_restore_sqlite` にディスパッチ |
| `:do_restore_bind_target` | `%1`=BIND index | `RESTORE_OK`/`RESTORE_FAIL`/`RESTORE_SKIP` | `BIND_%1` をパースしてバインドマウントデータをリストア |
| `:do_restore_settings_target` | `%1`=SETTINGS index | `RESTORE_OK`/`RESTORE_SKIP` | `SETTINGS_%1` をパースして設定ファイルをリストア |
...
| 6 | ポート競合検出 (18ポート) | WARN | `netstat` (bat) / `ss`+`netstat` (sh) |
...
| `PG_COUNT` | `2` | PostgreSQL ターゲット数 |
| `PG_1`〜`PG_N` | パイプ区切り文字列 | PostgreSQL バックアップ/リストア対象定義 |
| `SQLITE_COUNT` | `3` | SQLite ターゲット数 |
| `SQLITE_1`〜`SQLITE_N` | パイプ区切り文字列 | SQLite バックアップ/リストア対象定義 |
| `BIND_COUNT` | `1` | バインドマウントターゲット数 |
| `BIND_1`〜`BIND_N` | パイプ区切り文字列 | バインドマウントバックアップ/リストア対象定義 |
| `SETTINGS_COUNT` | `1` | 設定ファイルターゲット数 |
| `SETTINGS_1`〜`SETTINGS_N` | パイプ区切り文字列 | 設定ファイルバックアップ/リストア対象定義 |
| `BACKUP_TOTAL` | 計算値 | バックアップ/リストアの総ステップ数 |
