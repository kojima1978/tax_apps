...
**最終更新**: 2026-03-28
**バージョン**: 2.2（アプリ一覧整理、ポート一覧更新、stock-valuation-form本番ビルド対応）
...
複数の独立した Docker Compose プロジェクト（14アプリ）を統合管理するオーケストレーションスクリプト。
...
| 3 | docker-compose.yml 存在確認 (14個) | WARN | 各 `APP_*` パスの `docker-compose.yml` 存在チェック |
...
| 6 | ポート競合検出 (17ポート) | WARN | `netstat` (bat) / `ss`+`netstat` (sh) |
