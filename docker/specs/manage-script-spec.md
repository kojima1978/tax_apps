...
**最終更新**: 2026-05-07
**バージョン**: 3.3（restore/clean のデータ保護強化）
...
複数の独立した Docker Compose プロジェクト（14管理対象アプリ + 個別起動の income-tax-calc）を統合管理するオーケストレーションスクリプト。
...
| 14 | `APP_14` | `docker\gateway` | Nginx Gateway + Portal (Next.js) | 80/3000 |
...
[5-13] フロントエンドのみアプリ
[14] Gateway（全アプリのコンテナ名を upstream として参照するため最後）
```
...
| 3.1 | income-tax-calc は開発中のため、manage.sh の全体管理対象から除外。個別起動時のみ app 配下の Compose を使用 |
...
| 処理 | `APP_1` → `APP_14` の順に `docker compose up -d` |
| 後処理 | `status` を自動表示 |
...
manage.sh clean-cache [--all]
```

| 項目 | 説明 |
|:-----|:-----|
| 概要 | Docker Build Cache の確認付き削除 |
| デフォルト | 7日以上使われていない Build Cache のみ削除 |
| `--all` | 未使用の Build Cache をすべて削除 |
...
| `preflight_quick` | - | `start` 前の Docker Desktop / docker compose 起動確認 |
| `print_docker_disk_usage` | - | `docker system df` 表示 |
| `port_is_listening` | `port` | Linux/Windows 両対応の待受ポート検出 |
| `is_managed_compose_project` | `project` | Compose project が `APPS` 管理対象か判定 |
| `tax_apps_container_for_port` | `port` | 指定ポートを使う管理対象 Tax Apps コンテナ名を返す |
...
入力: search_string（ユーザー指定のアプリ名）
  │
  ├── 全 APP_1〜APP_14 のパスに対して findstr /i (bat) または *match* (sh) で部分一致検索
  │
...
| 選択方式 | 引数指定 or 対話選択（全体バックアップ形式のみ） |
| 確認 | Y/N プロンプト + 実行前の `pre-restore` 自動バックアップ |
...
リストア承認後、現在データを `docker/backups/pre-restore_YYYY-MM-DD_HHMMSS/` にバックアップしてから上書き処理に進む。事前バックアップに失敗した場合はリストアを中止する。現在データが存在しない場合は警告表示のみで続行可能。
対話一覧と直接指定では、`YYYY-MM-DD_HHMMSS` または `pre-restore_YYYY-MM-DD_HHMMSS` 形式のディレクトリのみをリストア対象にする。
...
| 7 | ポート競合検出 (17ポート) | WARN | `ss` + `netstat` + Windows `Get-NetTCPConnection` フォールバック。Tax Apps 自身の使用ポートは外部競合から除外 |
| 8 | ホストディスク空き容量 | WARN | 5GB 未満で警告 |
| 9 | Docker daemon メモリ | WARN | 4GB 未満で警告 |
| 10 | Docker ディスク使用量 | WARN | `docker system df` を表示 |

### 対象ポート一覧

```
80, 3000, 3001, 3002, 3003, 3004, 3007,
3010, 3012, 3013, 3014, 3015, 3016, 3017, 3020, 3022, 5432
```
...
`start` コマンド実行時は簡易版 `preflight_quick` が自動実行される。Docker Desktop と `docker compose` の利用可否のみ確認し、失敗時はエラー表示して中止する。
...
| `APPS` | 配列 (14要素) | `income-tax-calc` を除く管理対象アプリパス一覧 |
