...
[5-12] フロントエンドのみアプリ
[13] Gateway（全アプリのコンテナ名を upstream として参照するため最後）
```
...
| 3 | docker-compose.yml 存在確認 (13個) | WARN | `income-tax-calc` を除く管理対象の `docker-compose.yml` 存在チェック |
...
| 7 | ポート競合検出 (16ポート) | WARN | `ss` + `netstat` + Windows `Get-NetTCPConnection` フォールバック。Tax Apps 自身の使用ポートは外部競合から除外 |
