...
**最終更新**: 2026-03-08
**バージョン**: 2.0（manage.sh 全機能搭載、stock-valuation-form 統合）
...
複数の独立した Docker Compose プロジェクト（12アプリ）を統合管理するオーケストレーションスクリプト。
...
| 11 | `APP_11` | `apps\stock-valuation-form` | 株式評価明細書 (Vite) **開発中** | 3014 |
| 12 | `APP_12` | `docker\gateway` | Nginx Gateway + Portal | 80/3000 |
...
[5-11] フロントエンドのみアプリ
[12] Gateway（全アプリのコンテナ名を upstream として参照するため最後）
```
...
入力: search_string（ユーザー指定のアプリ名）
  │
  ├── 全 APP_1〜APP_12 のパスに対して findstr /i (bat) または *match* (sh) で部分一致検索
  │
...
| 3 | docker-compose.yml 存在確認 (12個) | WARN | 各 `APP_*` パスの `docker-compose.yml` 存在チェック |
...
| 6 | ポート競合検出 (16ポート) | WARN | `netstat` (bat) / `ss`+`netstat` (sh) |
...
3010, 3012, 3013, 3014, 3020, 3022, 5173
```
