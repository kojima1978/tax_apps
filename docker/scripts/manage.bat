...
set "APP_11=apps\stock-valuation-form"
set "APP_12=docker\gateway"
...
:: 6. Port conflicts
set "PORT_CONFLICT=0"
for /f "tokens=*" %%L in ('powershell -NoProfile -Command "$ports = @(80,3000,3001,3002,3003,3004,3005,3006,3007,3010,3012,3013,3014,3020,3022,5173); $lines = netstat -ano 2>$null; foreach ($p in $ports) { foreach ($l in $lines) { if ($l -match 'LISTENING' -and $l -match \":$p\s\") { Write-Output $p; break } } }"') do call :do_preflight_port %%L
if "!PORT_CONFLICT!"=="0" (
...
if "!MATCH_COUNT!"=="0" (
    ...
    echo [ERROR] Multiple apps match: "!SEARCH!"
    echo !MATCH_LIST!
    echo.
    echo Please use a more specific name.
    set "RESOLVED_DIR="
)
goto :eof
...
if !ERRORLEVEL! equ 0 (
    ...
    for %%N in ("!APP_PATH!") do set "MATCH_LIST=!MATCH_LIST!  %%~nxN"
)
goto :eof
