...
for /L %%I in (1,1,%APP_COUNT%) do (
    ...
    if exist "!COMPOSE_FILE!" (
        ...
        if !ERRORLEVEL! neq 0 (
            echo [ERROR]   !APP_NAME! の起動に失敗しました
        )
    ) else (
        for %%N in ("!APP_PATH!") do set "APP_NAME=%%~nxN"
        echo [WARN]    スキップ（docker-compose.yml なし）: !APP_NAME!
    )
)
echo.
...
:backup_itcm_volume
:: ボリューム名を特定（compose project name = inheritance-case-management）
